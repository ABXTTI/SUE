...
class ZkMachine(models.Model):
    ...
    def __init__(self, pool, cr):
        super(ZkMachine, self).__init__(pool, cr)
        self.env = None
    ...
    def download_attendance(self):
        ...
        for info in self:
            ...
            timeout = info.zk_timeout
            try:
                zk = ZK(machine_ip, port=zk_port, timeout=timeout, password=1, force_udp=False, ommit_ping=False)
            except NameError:
                raise UserError(_("Pyzk module not Found. Please install it with 'pip3 install pyzk'."))
            conn = self.device_connect(zk)
            if conn:
                ...
                if attendance:
                    for each in attendance:
                        ...
                        if atten_time != False and atten_time > datetime.strptime(info.zk_after_date,
                                                                                      '%Y-%m-%d %H:%M:%S'):
